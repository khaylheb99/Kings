...
export const TrackingModal: React.FC<TrackingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-800 rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 dark:border-slate-700 flex flex-col">
        ...
        {/* Search Bar */}
        <div className="p-4 sm:p-6 bg-sky-50/60 dark:bg-slate-900/60 border-b border-sky-100 dark:border-slate-700 space-y-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                className="w-full pl-4 pr-10 py-3 text-sm font-mono font-medium rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:outline-hidden focus:ring-2 focus:ring-[#152A4E]"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            <button
              type="submit"
              className="bg-[#152A4E] dark:bg-sky-600 hover:bg-[#1D4F91] dark:hover:bg-sky-500 text-white px-6 py-3 rounded-xl font-heading font-bold text-xs uppercase tracking-wider transition-all cursor-pointer shadow-xs"
            >
              SEARCH
            </button>
          </form>
          ...
          <div className="flex items-center gap-2 text-xs flex-wrap">
            <span className="text-gray-500 dark:text-slate-400 font-medium">Quick Select:</span>
              ...
              <button
                ...
                className={`font-mono text-[11px] px-2.5 py-1 rounded-lg border transition-all ${
                  activeShipment?.trackingNumber === s.trackingNumber
                    ? 'bg-[#152A4E] dark:bg-sky-600 text-white border-[#152A4E] dark:border-sky-500 font-bold'
                    : 'bg-white dark:bg-slate-800 text-[#152A4E] dark:text-sky-300 border-gray-300 dark:border-slate-600 hover:border-sky-300'
                }`}
              >
                {s.trackingNumber}
              </button>
        ...
        <div className="p-5 sm:p-6 space-y-6 flex-1 overflow-y-auto">
          {activeShipment ? (
            <div className="space-y-6">
              ...
              {/* Header Status Bar */}
              <div className="bg-gray-50 dark:bg-slate-900/60 p-4 sm:p-5 rounded-2xl border border-gray-200 dark:border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-extrabold text-lg sm:text-xl text-[#152A4E] dark:text-sky-300">
                      {activeShipment.trackingNumber}
                    </span>
                    <button
                      onClick={() => handleCopy(activeShipment.trackingNumber)}
                      className="p-1 rounded-md hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-500 dark:text-slate-400"
                      title="Copy"
                    >
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <span className="text-[10px] uppercase font-bold text-gray-400 dark:text-slate-400 block">Status</span>
                    <span className="font-heading font-extrabold text-sm text-[#2FAE60] bg-emerald-50 dark:bg-emerald-950/40 px-3 py-1 rounded-full border border-emerald-200 dark:border-emerald-800 inline-block">
                      {activeShipment.status}
                    </span>
                  </div>
                  <div className="text-right pl-3 border-l border-gray-200 dark:border-slate-700">
                    <span className="text-[10px] uppercase font-bold text-gray-400 dark:text-slate-400 block">Est. Delivery</span>
                    <span className="font-semibold text-xs text-[#152A4E] dark:text-slate-200">
              ...
              <div className="p-3.5 bg-sky-50 dark:bg-slate-900/60 rounded-xl border border-sky-200 dark:border-slate-700 flex items-center gap-3 text-xs text-[#152A4E] dark:text-sky-200">
                <MapPin className="w-5 h-5 text-[#1D4F91] dark:text-sky-400 shrink-0" />
                <div>
              ...
              <div className="space-y-4 pt-2">
                <h4 className="font-heading font-bold text-sm text-[#152A4E] dark:text-slate-200 uppercase tracking-wider">
                ...
                <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-200 dark:before:bg-slate-700">
                  {activeShipment.events.map((event, idx) => (
                    <div key={idx} className="relative flex items-start gap-4">
                      ...
                      {/* Event Details */}
                      <div className="flex-1 bg-white dark:bg-slate-800 p-3.5 rounded-xl border border-gray-200/80 dark:border-slate-700 shadow-2xs">
                        <div className="flex items-center justify-between text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">
                          <span className="text-[#1D4F91] dark:text-sky-300 font-bold">{event.location}</span>
                          <span>{event.date} • {event.time}</span>
                        </div>
                        <h5 className="font-heading font-bold text-sm text-[#152A4E] dark:text-slate-100">
                          {event.status}
                        </h5>
            ...
            <div className="text-center py-12 space-y-3">
              ...
              <h4 className="font-heading font-bold text-lg text-[#152A4E] dark:text-slate-200">
        ...
        {/* Footer */}
        <div className="p-4 bg-gray-50 dark:bg-slate-900/80 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between text-xs">
          <span className="text-gray-500 dark:text-slate-400">
            Kingz Logistics Houston Center • Hotline: +1 (832) 555-KINGZ
          </span>
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-[#152A4E] dark:bg-sky-600 text-white font-semibold hover:bg-[#1D4F91] dark:hover:bg-sky-500 transition-colors"
          >
            Close Portal
          </button>
        </div>
