...
export const ConfirmationScreen: React.FC<ConfirmationScreenProps> = ({
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in zoom-in-95 duration-200">
      ...
      {/* Top Banner Card */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-200 shadow-md text-center space-y-4 relative overflow-hidden">
        {/* Background accent */}
        ...
        <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-tr from-[#BFE3F7] to-[#8FCBEA] flex items-center justify-center p-2 shadow-inner">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-gray-600 max-w-xl mx-auto font-body">
            Thank you for trusting <strong className="text-[#152A4E]">Kingz Logistics</strong>. Your shipment request has been logged and our Houston dispatch team is preparing your consignment.
          </p>
        </div>

        {/* Generated Reference Number & Barcode Box */}
        <div className="bg-sky-50/80 border border-sky-200/80 rounded-2xl p-4 sm:p-5 max-w-md mx-auto space-y-2">
          <span className="text-xs font-bold uppercase tracking-wider text-gray-500 block">
            Shipment Tracking Reference
          </span>

          <div className="flex items-center justify-center gap-2">
            <span className="font-mono font-extrabold text-xl sm:text-2xl text-[#152A4E] tracking-wider select-all">
              {bookingRef}
            </span>
            <button
              onClick={handleCopyRef}
              className="p-2 rounded-xl bg-white border border-sky-300 text-[#152A4E] hover:bg-sky-100 transition-all flex items-center gap-1 text-xs font-semibold cursor-pointer"
          ...
          <div className="pt-2 flex flex-col items-center">
            <div className="h-9 w-48 bg-gray-900 opacity-90 rounded-xs flex items-center justify-between px-2 gap-1">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
          <button
            ...
            className="w-full sm:w-auto px-8 py-3 rounded-xl bg-[#152A4E] hover:bg-[#1D4F91] text-white font-heading font-bold text-sm flex items-center justify-center gap-2 shadow-sm cursor-pointer transition-all"
          >
          ...
          <button
            onClick={handlePrint}
            className="w-full sm:w-auto px-6 py-3 rounded-xl border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 font-heading font-semibold text-sm flex items-center justify-center gap-2 cursor-pointer transition-all"
          >
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-200 shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <h3 className="font-heading font-bold text-base text-[#152A4E] flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-[#2FAE60]" />
            Booking Confirmation Summary
          </h3>
          <span className="text-xs font-bold text-[#2FAE60] bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
            ✔ Confirmed & Logged
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          ...
          {/* Route & Cargo */}
          <div className="space-y-2 bg-gray-50 p-3.5 rounded-xl border border-gray-200/80">
            <span className="font-bold text-[#152A4E] uppercase text-[11px] block border-b border-gray-200 pb-1">
          ...
          <div className="space-y-2 bg-gray-50 p-3.5 rounded-xl border border-gray-200/80">
            ...
            <div className="space-y-1 text-gray-700">
              ...
              <div className="pt-1 border-t border-gray-200"><strong>Receiver:</strong> {formData.receiver.fullName} ({formData.receiver.phone})</div>
        ...
        {/* Cost & Payment */}
        <div className="bg-sky-50/80 p-4 rounded-xl border border-sky-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div>
            <span className="font-bold text-[#152A4E] block">Payment Selection: <span className="capitalize">{formData.paymentMethod.replace('_', ' ')}</span></span>
            <span className="text-gray-500">Insurance Protection: {formData.addInsurance ? '✔ Active ($' + rates.insuranceFee + ' USD)' : 'Not selected'}</span>
          </div>
          <div className="text-right">
            <span className="text-gray-500 block text-[11px]">Total Estimated Quote</span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
        <button
          onClick={onResetForm}
          className="text-xs font-bold text-[#152A4E] hover:text-[#1D4F91] underline cursor-pointer"
        >
